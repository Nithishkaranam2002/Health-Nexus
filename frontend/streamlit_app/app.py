...
from api import (
    login, get_health, summarize_records, patient_chat,
    analyze_image, extract_pdf_text,
)
...
with st.sidebar:
    ...
    if st.session_state.auth["token"]:
        st.success(f"Logged in as {st.session_state.auth['user']} ({st.session_state.auth['role']})")
        if st.button("Logout"):
            st.session_state.auth = {"token": None, "user": None, "role": None}
            do_rerun()
    else:
        ...
        role = st.selectbox("Role", ["clinician", "patient", "admin"])
        if st.button("Login"):
            try:
...
if role in ("clinician", "admin"):
    ...
    with col1:
        img_file = st.file_uploader("Upload medical image (PNG/JPG/DICOM)", type=["png","jpg","jpeg","dcm"])
    with col2:
        pdf_file = st.file_uploader("Upload clinical PDF (EHR/notes)", type=["pdf"])
    ...
    if img_file is not None and st.button("Analyze image"):
        with st.spinner("Analyzing image..."):
            try:
                res = analyze_image(img_file.read(), img_file.name, token=st.session_state.auth["token"])
    ...
    if pdf_file is not None and st.button("Extract text from PDF"):
        with st.spinner("Extracting text..."):
            try:
                out = extract_pdf_text(pdf_file.read(), pdf_file.name, token=st.session_state.auth["token"])
    ...
    if st.button("Summarize"):
        if not text.strip() and not findings:
            ...
            with st.spinner("Summarizing..."):
                try:
                    out = summarize_records(text, findings, token=st.session_state.auth["token"])
                    st.success("Summary")
                    st.write(out.get("summary", ""))
                    if out.get("citations"):
                        st.caption("Citations"); st.json(out["citations"])
                except Exception as e:
                    st.error(str(e))
    ...
    chosen = None
    if sug1: chosen = "What are the key problems and differentials based on this case?"
    if sug2: chosen = "Which labs and vitals should be monitored over the next week, and why?"
    if sug3: chosen = "What red flags would warrant escalation or imaging follow-up?"
...
# ======================================================================
    ...
    doc = st.file_uploader(
        "Discharge summary, instructions, or notes",
        type=["pdf","txt"],
    ...
    if extract_clicked and doc is not None:
        try:
            if doc.name.lower().endswith(".pdf"):
                ...
                st.session_state.patient_context_text = doc.read().decode("utf-8", errors="ignore")
